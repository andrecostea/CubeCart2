<div class="title_swipe"><h2>{$PRODUCT.name}</h2></div>
...
<form action="{$VAL_SELF}" method="post" class="addForm">
  <div>
	<div id="product_detail">
	  ...
	  <p class="buy_button">
		...
		<input type="submit" value="{$LANG.catalogue.buy_it}" class="button_blue" />
	  </p>
	  {else}
	  	{if $CTRL_HIDE_PRICES}
  ...
  <div class="title_swipe"><h2>{$LANG.catalogue.product_info}</h2></div>
  ...
  <div id="quantity_discounts">
	<div class="title_swipe"><h2>{$LANG.catalogue.quantity_discounts}</h2></div>
...
{if $CTRL_REVIEW}
<div class="title_swipe"><h2>{$LANG.catalogue.customer_reviews}</h2></div>
<div id="reviews">
  <div id="review_read">
  {if $REVIEWS}
